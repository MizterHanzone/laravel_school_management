                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <a href="{{ route('fee.create') }}" class="btn btn-primary"
                                    ...
                                    <tbody>
                                        @foreach ($fees as $fee)
                                            <tr>
                                                ...
                                                <td>{{ $fee->name }}</td>
                                                <td>
                                                    <a class="btn btn-success" href="{{ route('fee.edit', ['id' => $fee->id]) }}">Edit</a>
                                                    <form action="{{ route('fee.destroy', $fee->id) }}" method="POST"
                                                        class="d-inline" id="delete-form-{{ $fee->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="btn btn-danger"
                                                            onclick="confirmDelete({{ $fee->id }})">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
